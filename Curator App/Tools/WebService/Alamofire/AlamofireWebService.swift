...
public class AlamofireWebService: WebService {
    ...
    public final func make(request: WebRequest) -> WebHandler {
        ...
        let dataRequest = self.sessionManager.request(self.serverBaseURL.appendingPathComponent(request.path), method: method, parameters: request.params, encoding: encoding, headers: request.headers)
